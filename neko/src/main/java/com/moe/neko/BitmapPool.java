...
public class BitmapPool extends LinkedHashMap<Integer,Stack<Bitmap>> implements Resource.OnResourceListener
{
 long maxSize,currentSize;
    ...
    public void onResourceRelease(Resource<Image> res) {
        recycle(res.image.getBitmap());
    }

    public static BitmapPool getDefault(){
        if(pool==null)
            synchronized(BitmapPool.class){
                if(pool==null){
                    pool=new BitmapPool(Runtime.getRuntime().maxMemory()/2);
                }
            }
        return pool;
    }
